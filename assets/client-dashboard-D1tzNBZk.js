import{a as e}from"./supabase-_EgudPJk.js";import{a as t,i as n,n as r,r as i,t as a}from"./index-D9k83fD8.js";var o=t(a(),1);async function s(t,a,{user:s,profiles:c}){if(!c||c.length===0){a.innerHTML=`
    ...
    `,document.querySelectorAll(`.biz-tab`).forEach(e=>{e.onclick=()=>{l=parseInt(e.dataset.idx),u()}}),document.getElementById(`cp-logout`).onclick=async()=>{await e.auth.signOut(),n(`/`)},document.querySelectorAll(`.cr-btn[data-reward-id]`).forEach(n=>{n.onclick=async()=>{let r=parseInt(n.dataset.rewardPts),a=n.dataset.rewardName,o=n.dataset.rewardId;if(!confirm(`¿Canjear "${a}" por ${r} puntos?`))return;n.disabled=!0,n.textContent=`Canjeando...`;let{data:d,error:f}=await e.from(`customers`).select(`id, points, user_id`).eq(`id`,t.id).single();if(f||!d){console.error(`[Redeem] Fetch error:`,f),i(`Error al verificar puntos.`,`error`),n.disabled=!1,n.textContent=`¡Canjear!`;return}if(d.points<r){i(`Puntos insuficientes.`,`error`),n.disabled=!1,n.textContent=`${r-d.points} más`;return}let p=d.points-r,{data:m,error:h}=await e.from(`customers`).update({points:p}).eq(`id`,d.id).eq(`user_id`,d.user_id).select(`id, points`);if(h){console.error(`[Redeem] Update RLS error:`,h),i(`Error al actualizar puntos: ${h.message}`,`error`),n.disabled=!1,n.textContent=`¡Canjear!`;return}if(!m||m.length===0){console.error(`[Redeem] RLS BLOCKED update — no rows affected. Apply docs/fix-rls-canjes.sql in Supabase dashboard.`),i(`❌ Error de permisos. Contactá al administrador.`,`error`),n.disabled=!1,n.textContent=`¡Canjear!`;return}let{error:g}=await e.from(`point_transactions`).insert({business_id:s.id,customer_id:t.id,type:`redeem`,points:-r,reward_id:o||null});g&&console.error(`[Redeem] TX insert error:`,g),c[l]={...c[l],points:p},i(`¡Canjeaste "${a}"! 🎁 (-${r} pts)`),u()}});let g=s.qr_ttl_seconds||90,_=g,v=null;async function y(){let e=await r(s.id,t.id,g),n=document.getElementById(`cp-qr-canvas`),i=document.getElementById(`cp-qr-token-txt`);n&&(e?(await o.toCanvas(n,e,{width:180,color:{dark:`#0a0a0a`,light:`#ffffff`}}),i&&(i.textContent=e)):i&&(i.textContent=`Error generando QR`))}function b(){clearInterval(v),document.getElementById(`cp-qr-overlay`)?.classList.add(`hidden`),_=g;let e=document.getElementById(`cp-qr-countdown`);e&&(e.textContent=_),v=setInterval(()=>{_--;let e=document.getElementById(`cp-qr-countdown`);e&&(e.textContent=_),_<=0&&(clearInterval(v),document.getElementById(`cp-qr-overlay`)?.classList.remove(`hidden`))},1e3)}async function x(){await y(),b()}document.getElementById(`cp-btn-renew`)?.addEventListener(`click`,x),x(),document.getElementById(`fab-show-qr`).onclick=()=>{let e=document.querySelector(`.qr-box`);e&&(e.scrollIntoView({behavior:`smooth`,block:`center`}),e.style.boxShadow=`var(--primary-glow-lg)`,setTimeout(()=>e.style.boxShadow=`var(--primary-glow)`,1500))}}u()}export{s as renderClientDashboard};